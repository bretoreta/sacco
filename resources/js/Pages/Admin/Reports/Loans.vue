...
const applyFilters = () => {
    ...
    axios.post(route('admin.reports.loans.filter'), form)
        .then((res) => {
            usePage().props.loans = res.data
...
const submitSearch = () => {
    ...
    form.reset();
    axios.post(route('admin.reports.loans.search'), search)
        .then((res) => {
            usePage().props.loans = res.data
            usePage().props.flash.message = {
                    type: 'success',
...
const resetFilters = () => {
    form.reset();
    router.visit(route('admin.reports.loans.index'));
}
...
<template>
    <AdminLayout title="Loans Report">
        <div class="py-6 md:py-12">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="grid grid-cols-12 gap-5 px-4">
                    <div class="col-span-12">
                        <div class="pb-10">
                            <h4 class="text-3xl">Loans Report</h4>
                            <div class="mt-5">
                                <div class="w-full">
                                            ...
                                            <template #content>
                                                <div class="p-5 w-full">
                                                    ...
                                                    <div class="flex flex-col space-y-3">
                                                        <template v-for="item in loan_types" :key="item.id">
                                                            <div class="flex items-center">
                                                                <input v-model="form.loan_type" :id="item.id" type="radio" :value="item.loan_type" name="default-radio" class="w-4 h-4 text-blue-500 bg-gray-100 border-gray-300 focus:ring-blue-400 focus:ring-2">
                                                                <label :for="item.id" class="ml-2 text-sm font-medium text-gray-900">{{ item.loan_type }}</label>
                                                            </div>
                                                        </template>
                                                    </div>
                                                </div>
                        ...
                        <div id="loans-report">
                            <div class="bg-white dark:bg-gray-800">
                                <div
                                    ...
                                    loans Report
                                    <p class="mt-1 text-sm font-normal text-gray-500 dark:text-gray-400">Compiled report of user loans records in the system</p>
                                </div>
                            </div>
